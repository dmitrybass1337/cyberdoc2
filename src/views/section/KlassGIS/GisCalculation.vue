...
<p><strong>Обработка персональных данных: </strong><span class="text-success"><strong>{{ systemPers }}</strong></span></p>
      <!-- Блок выбора сегментации, если тип ГИС -->
        <h5>Выберите тип сегментации:</h5>
        <div class="form-check">
          <input class="form-check-input" type="radio" id="segment" value="Сегментная" v-model="segmentationType" />
          <label class="form-check-label" for="segment">Сегментная</label>
        </div>
        <div class="form-check">
          <input class="form-check-input" type="radio" id="closed" value="Закрытая" v-model="segmentationType" />
          <label class="form-check-label" for="closed">Закрытая</label>
        </div>
...
// реализация перехода на следующую страницу
  ...
  setup(props, { emit }) {
const goNextStepGIS = () => {
  emit('next-step-gis', levels, currentClass.value, currentMashtab.value);
